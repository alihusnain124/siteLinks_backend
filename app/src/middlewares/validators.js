...
export const handleValidationErrors = (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    return errorResponse(res,400,'Validation failed',errors.array().map(err => ({
        field: err.param,
        message: err.msg,
      })),)
  }
...
export const siteValidators = {
  ...
  update: [
    param('id')
      .isUUID()
      .withMessage('Invalid site ID (must be a valid UUID)'),
  ...
  getById: [
    param('id')
      .isInt({ min: 1 }).withMessage('Invalid site ID'),
